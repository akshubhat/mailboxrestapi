...
class DbHandler {
    ...
    public function createmessage($user_id, $message) {
        $stmt = $this->conn->prepare("INSERT INTO messages(message) VALUES(?)");
    ...
    public function getmessage($message_id, $user_id) {
        $stmt = $this->conn->prepare("SELECT t.id, t.message, t.status, t.created_at from messages t, user_messages ut WHERE t.id = ? AND ut.message_id = t.id AND ut.user_id = ?");
        $stmt->bind_param("ii", $message_id, $user_id);
        if ($stmt->execute()) {
            $res = array();
            $stmt->bind_result($id, $message, $status, $created_at);
            // TODO
    ...
    public function getAllUsermessages($user_id) {
        $stmt = $this->conn->prepare("SELECT t.* FROM messages t, user_messages ut WHERE t.id = ut.message_id AND ut.user_id = ?");
        $stmt->bind_param("i", $user_id);
        $stmt->execute();
     ...
     */
    public function updatemessage($user_id, $message_id, $message, $status) {
        $stmt = $this->conn->prepare("UPDATE messages t, user_messages ut set t.message = ?, t.status = ? WHERE t.id = ? AND t.id = ut.message_id AND ut.user_id = ?");
        $stmt->bind_param("siii", $message, $status, $message_id, $user_id);
        $stmt->execute();
    ...
    public function deletemessage($user_id, $message_id) {
        $stmt = $this->conn->prepare("DELETE t FROM messages t, user_messages ut WHERE t.id = ? AND ut.message_id = t.id AND ut.user_id = ?");
        $stmt->bind_param("ii", $message_id, $user_id);
        $stmt->execute();
        $num_affected_rows = $stmt->affected_rows;
        $stmt->close();
